...
- 如果候选搜索工具返回空结果：
  ...
  - 必须向用户索要更准确的信息，或在参数规则允许时询问是否新增自定义项
...
- 本能力涉及的候选搜索工具包括：
  ...
  - `tool.inventory_tag_list`
- 对允许新增自定义项的参数：
  - 成本类型、售价类型、回收类型、标签在候选为空时，不要反复搜索
  - 必须先让用户确认是否新增自定义项
  - 用户确认新增后，按对应参数规则调用新增工具
- 对品牌、系列、型号：
  - 搜索无结果时，不要自动拆分品牌词、系列词、型号词循环查询
  - 必须让用户补充更准确的品牌、系列、型号、图片或描述
  - 如果用户改用图片识款或智能填充基础信息，属于新的获取路径，可以按对应参数规则执行一次
- 对回收人员和商品位置：
  - 搜索无结果时，不要自行模糊匹配人员或仓库库区
  - 必须让用户补充更准确的姓名、手机号、仓库名或库区名
