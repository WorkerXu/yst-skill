...
const file = path.join(os.homedir(), '.mcporter', 'mcporter.json');
...
json.mcpServers.eshetang = { url, transportType: 'streamable-http' };
fs.writeFileSync(file, JSON.stringify(json, null, 2) + '\n', 'utf8');
NODE
printf '小龙虾/mcporter MCP 安装完成。\nMCP URL: %s\n配置文件: %s\n' "$MCP_URL" "$HOME/.mcporter/mcporter.json"
